...
final class QuestionFactory: QuestionFactoryProtocol{
    ...
    DispatchQueue.global().async { [weak self] in
       ...
       do {
        ...
        let lessOrMore = ["больше", "меньше"]
        let randomOperator = lessOrMore.randomElement()
        let text = "Рейтинг этого фильма \(randomOperator ?? "") чем \(randomRating)?"
        let correctAnswer: Bool
        if randomOperator == "больше" {
